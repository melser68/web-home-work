...
def phone_menu():
    ...
    print(
        '"Add" - adding a contact to the phonebook\n"Change" - change an existing contact'
    ...
    if chois == "Add":
        ...
        if chois.lower() == "yes":
            ...
            print(
                "Add contact's date of the birth un the format 'dd-mm-yyyy'(may be empty)"
            )
            date_birth = input(">>>>  ")
        ...
        while status == True:
            ...
            if len(result) == 0:
                ...
                new_value = input(">>>>  ")
                book.change_record(
                    list(result.keys())[int(num_change) - 1],
                    new_value,
                    list(
                        list(result.values())[int(num_change) - 1][
                            int(num_value) - 1
                        ].keys()
                    )[0],
                )
